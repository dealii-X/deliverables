...
\begin{deliverableTable}
    \tableEntry{Abstract}{ dealii-X D2.7 reports on the deployment of the
        ...
        presented in D2.6. The platform, named DealiiX Platform, is a desktop
        application providing a node-based low-code editor for building
        computational graphs, which are executed by CORAL, a C++ computational
...
visualization services. Section 7 reports on packaging, deployment and
quality assurance, and section 8 outlines the activities planned for the
...
\begin{figure}
    \centering
    \figureOrPlaceholder{canvas.png}{the canvas editor with a deal.II graph
        (e.g.\ the step-4 Poisson solver), the node palette on the left and
        the parameters panel on the right}
    \caption{The DealiiX Platform canvas editor}
    \label{canvas}
\end{figure}
...
\begin{figure}
    \centering
    \figureOrPlaceholder{subnetwork.png}{a subnetwork node on the canvas and
        the same subnetwork opened for editing, with the breadcrumb bar
        visible}
    \caption{Subnetwork nodes: a collapsed high-level node (no-code view)
        and its internal low-code definition}
    \label{subnetwork}
\end{figure}
...
execution monitoring described in section 5 to work transparently across
nested graphs.
...
persistent jobs table showing scheduler status (pending, running, completed,
failed, cancelled), submission and completion times, and the backend used.
...
\begin{figure}
    \centering
    \figureOrPlaceholder{jobs.png}{the jobs table with per-node execution
        status modal opened}
    \caption{Job monitoring: jobs table and per-node execution status}
    \label{jobs}
\end{figure}

Two deployment targets have been exercised during the testing phase. The
...
installation and an SSH server, faithfully simulating a remote HPC cluster
on the developer's machine. The second is a real remote machine: the same
containers are deployed on a remote host and reached through an SSH tunnel,
with a documented step-by-step setup guide. Since the application only
assumes SSH access and a Slurm scheduler, the same mechanism applies to
...
Simulation results are inspected through the Coral Visualizer, a web-based
viewer for VTK/deal.II output formats (\texttt{.vtk}, \texttt{.vtu}),
reachable directly from the application. During the reporting period the
...
the development plan presented in D2.6 (Fig.~\ref{gantt}) is completed. The
...
\begin{itemize}
    \item enrichment of the node registries with the dealii-X application
        ...
        the project;
    \item deployment and validation of the remote execution modes on
        production HPC systems;
