...
\begin{deliverableTable}
    ...
    \tableEntry{Deliverable version}{0.3}
...
(\texttt{lifex}, \texttt{.prm} input) and lungs (\texttt{exadg}, JSON input)
applications comply with the same parameter-file convention and are expected
...
What the tool adds is the combination of the two capabilities in one place.
Tagging a mesh is well established practice: a mesh generator such as Gmsh
lets the identifiers be defined together with the geometry, through its
physical groups, and this is the usual route when a mesh is built from a
geometric description. That route is not available when the mesh comes from
elsewhere, as is the case for the organ geometries reconstructed from medical
images, and the tags then have to be assigned by iterating over the cells with
geometric predicates, in the application code or in a conversion script, which
is easy to get wrong and gives no visual confirmation of the result.
Conversely a ParaView-based backend brings a rich and composable set of
filters, and the ability to work on datasets larger than a single workstation
can hold, but it offers no direct way to write a value onto an interactively
picked selection. Combining the two makes the tags assignable, and immediately
verifiable, in the same environment in which the mesh is filtered and
inspected.
