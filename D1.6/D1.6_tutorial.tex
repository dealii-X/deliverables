...
\begin{documentControl}
  ...
  \addVersion{0.3}{Feb 27th 2026}{Salvatore Filippone}{Reorganization
    of sections}
  \addVersion{1.0}{Feb 28th 2026}{Salvatore Filippone}{Release}
\end{documentControl}
...
especially PSCToolkit~\url{https://psctoolkit.github.io} and MUMPS~\url{https://mumps-solver.org/index.php}.
...
the headers which contains the interface to PSBLAS and AMG4PSBLAS.
...
The assembly loop is performed as in any standard tutorial program of deal.II. It
consists of a loop over all the cells owned by the current process,
scattering local contributions to the global system matrix and
right-hand side vector. The implementation of the assembly loop 
can be found in the \texttt{assemble\_system()} member function, which
is to all extends identical to many other tutorial programs. 
...
solution of the linear system. As previously mentioned, the symmetry and positive definiteness of the bilinear form
...
\texttt{SolverCG} class, which implements the PCG method, and is templated on the vector type, which is in this case \texttt{PSCToolkit::Vector<double>}. The workflow is shown in Listing \ref{lst:linear_solve} where, for the sake of clarity, we omit the preconditioner settings, which are reported
in Listing \ref{lst:preconditioner_setup}.

Among the several available configuration options, we highlight the
integration with the MUMPS package (cfr. deliverable D1.7), which can
be employed as a direct coarse-level solver within the AMG
preconditioner. This combination is particularly effective for
achieving robust convergence when algebraic multigrid methods benefit
from an accurate solution at the coarsest level. Moreover,
parallel-efficient coarse solvers can improve the overall scalability
of the solver, especially in contexts where the size of coarse-level
system is large. 


The solver setup thus remains unchanged with respect to standard user
codes; only the AMG4PSBLAS preconditioner settings (coarsening
strategy, coarse solver, smoothers, tolerances) may require tuning by
users. 
\newpage
...
\begin{equation}
  ...
  \end{pmatrix},
\end{equation} and right-hand side $f=1$. The CG solver is stopped when the relative residual is reduced to a factor $10^{-6}$ or less compared to the initial residual, or when the absolute residual is smaller than $10^{-10}$.
