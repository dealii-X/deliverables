...
\begin{documentControl}
    ...
    \addVersion{0.2}{July 18, 2026}{Ivan Prusak}{Intermediate review and adjustments}
    \addVersion{0.3}{July 30, 2026}{Marco Feder}{Update with PSCToolkit}
    \addVersion{1.0}{July 31, 2026}{Ivan Prusak}{Final review and adjustments}
\end{documentControl}
...
The dealii-X project develops an exascale-ready simulation environment for
computational medicine, built on the open-source finite element library
\texttt{deal.II}. Work Package 1 is dedicated to the library itself: it provides the
numerical and computational infrastructure on which the organ-scale simulators
developed in the other work packages rely. The work is organised in five tasks, each
led by a different partner and each addressing a distinct layer of that
infrastructure: the exascale capabilities of the core matrix-free framework (RUB), the
improvement of pre-exascale modules (UNIPI), an experimental polygonal discretization
module (SISSA), and the integration of two external solver packages, PSCToolkit
(UNITOV) and MUMPS (INPT).

The present document is the fourth in the series of semestral reports on these
activities and covers the developments carried out during the fourth semester of the
project. During this period the emphasis shifted from prototyping towards
consolidation: several components previously developed in external repositories were
prepared for, or merged into, the \texttt{deal.II} core library, and the benchmark
suites assembled in earlier semesters were used to quantify the performance of the
resulting implementations on the JUPITER, LUMI, MareNostrum 5 and CALMIP systems.
The results reported here therefore combine algorithmic developments, software
integration work, and performance measurements on current European pre-exascale and
exascale hardware.

\subsection{{Purpose of the Document}}
This document reports the technical work carried out in Work Package 1 during the
fourth reporting semester. For each task it describes the algorithmic and
implementation developments, the software artefacts produced -- pull requests to
\texttt{deal.II} and to its Code Gallery, benchmark repositories and tutorial
programs -- and the performance results obtained on the target architectures. Where
an activity is documented in detail elsewhere, in particular in Deliverables D1.8 and
D3.4, the relevant deliverable is referenced rather than reproduced.
...
The matrix-free framework and its initial implementations are described in Deliverable D1.3 (\url{https://github.com/dealii-X/deliverables/blob/main/D1.3/D1.3_dealii_report.pdf}).
...
Elements designed for this purpose are known as Raviart-Thomas finite elements. We developed the vector Mass, mixed divergence and mixed gradient operators, which are the core operations
...
particular, we consider the cph-MG variant, which is the most performant one on CPUs: the discontinuous space is first embedded into the corresponding continuous space on the finest level, and the resulting problem is then treated by the ph-multigrid solver developed previously for continuous elements~\cite{Fehn2020}. The performance of the mixed-precision cph-MG solver is shown in Figure~\ref{fig:cph_mg}.
