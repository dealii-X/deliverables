...
An example of the APS output is shown in Fig.~\ref{fig:intel_aps}.
...
In tests targeting only a single GPU device, this results in ``artificially'' low
utilization.
...
\begin{lstlisting}[
    ...
    backgroundcolor=\color{backcolour},
    caption={Output of \texttt{likwid-mpirun} for a BP3-inspired benchmark on an Intel Xeon Platinum 8360Y CPU (2 sockets, 72 cores used).}]
+----------------------------------------+--------------+------------+
|                 Metric                 |      Sum     |     Min    |
+----------------------------------------+--------------+------------+
...
p3em operates by spawning a background thread that manages a power-measu\-rement loop.
This thread initiaties a sub-process which interfaces with vendor-specific APIs or
OS-specific energy measurement tools (such as the NVIDIA tool \texttt{nvidia-smi} or Linux \texttt{perf})
...
This report has documented the workflow established in dealii-X for
performance measurements and tuning. We have made considerable progress in
reaching hardware limits in a range of benchmarks, and will further build on
this basis for the upcoming developments.
