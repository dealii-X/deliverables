...
The p3em\footnote{\url{https://github.com/svtcli/p3em}} library \cite{Cielo2025} provides a modular framework for measuring power and energy across heterogeneous devices. It is designed with a minimalistic, header-only C++ API, facilitating
easy integration into existing codebases. The API is exposed as a single class
named \texttt{p3em}, instances of which serve as ``energy-meters''.

The p3em library operates by spawning a background thread that manages a measurement loop.
This thread initiaties a sub-process which interfaces with vendor-specific APIs
(such as NVIDIA's \texttt{nvidia-smi} or Linux \texttt{perf}) to periodically
query the device energy usage.
...
The subprocess is responsible for the temporal integration of power into energy.
It must output a monotonically increasing energy value, representing the total
Joules consumed.
Typically, this is calculated via trapezoidal integration
of instantaneous power samples\footnote{In practice, these values may already be averaged by the device driver}.
Alternatively, the sub-process may directly report hardware-level energy
counters if supported by the device.
The background thread in the parent process captures these value from the
sub-process output, providing a synchronized running total for the given
region of interest.
The \texttt{p3em.sh} script provided in the library's reposotiry serves
...
using p3em. Further usage examples in C, Fortran and Python can be found at the p3em source repository.
